...
function Reply({
  ...
  const [replyNull, setReplyNull] = useState(false);
  ...
  useEffect(() => {
    ...
    replyData.length > 0 && replyNum > 0 ? setLoading(false) : replyDataNull();
  }, [loading, replyData]);
  const replyDataNull = () => {
    setLoading(true);
    setTimeout(() => {
  ...
  const changeReply = () => {
    let replyValue = document.getElementsByName('reply-write')[0].value.trim();
  ...
  const getReplyData = async (board_id) => {
    const data = await axios(
      'https://delivery-mate.herokuapp.com/get/reply_data',
      {
        method: 'POST',
        headers: new Headers(),
  ...
  const addReply = async (e) => {
    ...
    await axios('https://delivery-mate.herokuapp.com/add/reply', {
      method: 'POST',
      headers: new Headers(),
      data: data,
    });
  ...
  const removeReply = async (reply_id) => {
    if (window.confirm('해당 댓글을 삭제하시겠습니까?')) {
      await axios('https://delivery-mate.herokuapp.com/delete/reply', {
        method: 'POST',
        headers: new Headers(),
    ...
    <ReplyContainer>
      <PhoneBox>
        ...
        <ReplyWrite onSubmit={addReply}>
          ...
          <input type='submit' value='등록' id='reply-submit-btn'></input>
        </ReplyWrite>
      </PhoneBox>
    </ReplyContainer>
  );
